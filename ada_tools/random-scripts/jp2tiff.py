            ...
            for path in Path(os.path.join(dir, prepost + "-event")).rglob("*.zip"):
                if AOI in path.name:
                    target_path = os.path.join(
                        dir, AOI, prepost + "-event", str(path.name)
                    )
                    subprocess.run(f"cp {str(path)} {target_path}", shell=True)
                    subprocess.run(
                        f"unzip {target_path} -d {os.path.join(dir, AOI, prepost+'-event')}",
                        shell=True,
                    )
            ...
            for path in Path(os.path.join(dir, AOI, prepost + "-event")).rglob("*.JP2"):
                ...
                if dest == "":
                    ...
                    path_tif = os.path.join(
                        dest, str(path.name).replace(".JP2", ".tif")
                    )
                print(path.name)
                subprocess.run(
                    f"gdal_translate -of GTiff -co TILED=YES {path_jp2} {path_tif}",
                    shell=True,
                )
