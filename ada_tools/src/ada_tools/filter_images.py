...
def filter_by_ntl(country, ntl_shapefile, dest):
    ...
    if country != "":
        country_ntl_shapefile = (
            ntl_shapefile.split(".")[0] + "_" + country.lower() + ".shp"
        )
...
@click.option(
    "--ntl", default=False, help="filter pre-event images by night-time lights (yes/no)"
)
@click.option(
    "--ntl-shapefile",
    default=None,
    help="path to ntl shapefile, only needed with --ntl flag",
)
@click.option(
    "--bbox", default="", help="filter pre-event images by bounding box (CSV format)"
)
