...
def download_blob(container, blobpath, filepath, secret):
    ...
    blob_client = blob_service_client.get_blob_client(
        container=container, blob=blobpath
    )
...
def get_extent(raster: str) -> gpd.GeoDataFrame:
    ...
    gdf = gpd.GeoDataFrame(
        {"geometry": df.geometry.tolist(), "file": df.file.tolist()}, crs=crs_proj
    )
...
@click.option(
    "--refbuilds", default="buildings.geojson", help="input reference buildings"
)
...
def main(ext, builds, container, secret, raster, refbuilds, dest):
    ...
    gdf_builds_extents = gdf_builds_extents.rename(
        columns={"file": "alternative_buildings_file"}
    )
