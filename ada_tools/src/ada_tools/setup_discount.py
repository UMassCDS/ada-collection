...
def get_tile_bounds(tile_id):
    ...
    # print(f"Debug: Created Tile -> {tile}", flush=True)
    result = bounds(tile)
    # print(f"Debug: Calculated bounds -> {result}", flush=True)
    return result
...
def get_image_dir_contents(image_dir):
    ...
    for tif_path in dir_path.glob("*.tif"):
        ...
        with rasterio.open(tif_path) as geo_image_file:
            tiff_bounds = transform_bounds(
                geo_image_file.crs, "epsg:4326", *geo_image_file.bounds
            )
...
@click.option(
    "--outdir", default="tiles", help="directory containing all tile geojsons"
)
@click.option(
    "--out-csv",
    default="annotations.csv",
    help="file containing all tile ids and annotator info",
)
def main(input, pre_images, post_images, outdir, out_csv):
    ...
    tiles_df = (
        gdf.groupby(tile_id_key)["damage"]
        .count()
        .reset_index()
        .rename(columns={"damage": "detector_count"})
    )
    tiles_df.loc[:, ["true_count", "Annotator"]] = ""
    detector_counts = list(tiles_df["detector_count"])
    if min(detector_counts) == 0:
        detector_counts = [
            c + max(max(detector_counts) * EPS, 1) for c in detector_counts
        ]
    ...
    samples = list(
        np.random.choice(tile_ids, 2 * len(tile_ids), p=prob_dist, replace=True)
    )
    ...
    display_order = (
        sample_tracking_df.groupby(tile_id_key)["ordering"].min().reset_index()
    )
    tiles_df = pd.merge(
        tiles_df, display_order, how="left", on=tile_id_key
    ).sort_values(by="ordering")
    tiles_df.to_csv(
        out_csv,
        index=False,
        columns=[tile_id_key, "detector_count", "true_count", "Annotator"],
    )

    # Collect sample indices to be written to each tile's geoJSON file
    tile_to_sample_map = (
        sample_tracking_df.groupby(tile_id_key)["ordering"].apply(list).to_dict()
    )
