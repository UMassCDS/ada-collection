...
for index, way in tqdm(enumerate(r.ways)):
    ...
    if coordinates[0] == coordinates[-1]:
        ...
        geopandas_dataframe.loc[index_start + index_g, "OBJECTID"] = id.findall(
            str(way).strip()
        )
        index_g += 1

geopandas_dataframe = geopandas_dataframe[
    ~(geopandas_dataframe.geometry.is_empty | geopandas_dataframe.geometry.isna())
]
